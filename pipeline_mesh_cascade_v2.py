#!/usr/bin/env python3
"""
pipeline_mesh_cascade_v2.py - converged avatar mesh cascade, iteration 1.
...
Tail geometry:
  Frames with no observed MP geometry get the same canonical 468v/898f
  topology posed from the v17 fusion head pose/anchor. These frames are
  explicitly tagged as pose_posed, mesh_conf=0, geometry_observed=False.
...
No FLAME / profile fit is built in this iteration. Hooks are left in the
metadata for an iteration-2 remap.
"""
...
STREAM_PATH = f"{OUT_DIR}/mesh_cascade_v2_stream.npz"
REPORT_PATH = f"{OUT_DIR}/mesh_cascade_v2_report.json"
NOTES_PATH = f"{OUT_DIR}/notes_mesh_cascade_v2.md"
MONTAGE_PATH = f"{OUT_DIR}/mesh_cascade_v2_montage.png"
OVERLAY_MASTER_PATH = f"{OUT_DIR}/mesh_cascade_v2_overlay_master.mp4"
OVERLAY_PREVIEW_PATH = f"{OUT_DIR}/mesh_cascade_v2_overlay_preview.mp4"

PIPELINE_VERSION = "mesh_cascade_v2"
LOG_PREFIX = "[mesh-cascade-v2]"
...
def build_tests_and_report(verts: np.ndarray, faces: np.ndarray, projected_px: np.ndarray,
    ...
    mouth = {
        "pearson_jawOpen_vs_inner_lip_aperture_observed": pearsonr_np(jaw[observed], aperture[observed]),
        "pearson_jawOpen_vs_inner_lip_aperture_frontal_abs_yaw_lt_30": pearsonr_np(jaw[frontal], aperture[frontal]),
        ...
        "pass_observed_r_ge_0p75": bool(pearsonr_np(jaw[observed], aperture[observed]) >= 0.75),
        "pass_frontal_r_ge_0p85": bool(pearsonr_np(jaw[frontal], aperture[frontal]) >= 0.85),
        "kill_r_lt_0p70": bool(pearsonr_np(jaw[observed], aperture[observed]) < 0.70),
    }
    ...
    motion = {
        ...
        "pass_frontal_and_three_quarter_ge_8": bool(front_len >= 8 and tq_len >= 8),
    }
    ...
    return {
        ...
        "iteration2_hooks": {
            "flame_remap": "deferred",
            "profile_3d_fit_3ddfa_v2": "deferred_kill_gated",
        },
    }
...
def write_notes(report: Dict) -> None:
    ...
    lines = [
        ...
        f"- Observed: `{cov['observed_frames']}/{cov['total_frames']}` ({cov['observed_pct']:.1f}%). Full MP={cov['observed_full_mp']}, zoom MP={cov['observed_zoom_mp']}.",
    ...
    for bin_name, data in cov["by_yaw_bin"].items():
        lines.append(
            f"  - `{bin_name}`: total={data['total']} observed={data['observed']} predicted={data['predicted']}"
        )
    lines.extend([
        f"- Topology: V={topo['vertex_count']} F={topo['face_count']} faces_sha256=`{topo['faces_sha256']}` constant={topo['pass']}.",
        f"- Mouth/expr Pearson r observed: `{mouth['pearson_jawOpen_vs_inner_lip_aperture_observed']:.4f}` target >=0.75.",
    ...
    lines.extend([
        f"- Boundary pop: transitions={bp['n_transitions']} mean={bp['mean_jump_over_head_scale']:.4f} max={bp['max_jump_over_head_scale']:.4f}; target <=0.15, kill >0.25.",
        f"- Device: torch={device['torch_device']}; MediaPipe={device['mediapipe']}; YOLO={device['yolo_face']}.",
        f"- Disallowed call scan pass: `{report['mps_no_cuda']['pass']}` hits={report['mps_no_cuda']['hits']}.",
        f"- Verify in motion: frontal run={motion['frontal_longest_observed_run']}; 3/4 run={motion['three_quarter_longest_observed_run']}; pass={motion['pass_frontal_and_three_quarter_ge_8']}.",
        "",
        "## Kill Conditions",
...
def run() -> Dict:
    ...
    for fidx in range(total_f):
        ...
        if obs is not None:
            ...
            source_changed = fidx > 0 and not bool(geometry_observed[fidx - 1])
            ...
            source_changed = fidx > 0 and bool(geometry_observed[fidx - 1])
            current_verts, transition_diag = clamp_transition_if_needed(
                candidate_verts,
        ...
        if fidx % 50 == 0 or fidx == total_f - 1:
            ...
            print(
                f"{LOG_PREFIX} f{fidx}/{total_f}: full={n_full} zoom={n_zoom} "
    ...
    np.savez_compressed(
        ...
        profile_3d_fit=np.asarray(["deferred_iteration_2"], dtype="<U40"),
    )
    ...
    print(f"  Jaw r observed: {report['mouth_expr']['pearson_jawOpen_vs_inner_lip_aperture_observed']:.4f}")
